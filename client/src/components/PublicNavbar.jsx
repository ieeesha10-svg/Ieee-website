        ...
        <div className="md:hidden absolute top-16 left-0 w-full bg-navbar-background shadow-xl border-b border-gray-100 dark:border-gray-700 flex flex-col px-4 py-4 mt-4 gap-4 z-40">
          {NAV_LINKS.map((nav_link, index) => {
            return (
              <a
                ...
                onClick={closeMenu}
                className="text-foreground hover:text-primary-light transition flex items-center"
              >
                {nav_link.label}
              </a>
            );
          ...
          {user ? (
            <>
              ...
              {["admin", "board", "xcom"].includes(
                ...
                <Link
                  to="/dashboard"
                  onClick={closeMenu}
                  className="text-primary dark:text-sky-400 font-bold"
                >
            ...
            <>
              <Link
                to="/signup"
                className="text-lg px-6 py-3 w-fit bg-primary-linear text-white rounded-lg shadow-lg font-light"
              >
                Join Now
              </Link>
            </>
